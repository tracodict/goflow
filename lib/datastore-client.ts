...
export async function getDataSourceSchema(flowServiceUrl: string, id: string): Promise<DatasourceSchemaResponse> {
  const response = await authFetch(`${flowServiceUrl}/api/datasources/${id}/schema`, {
    method: 'GET'
  })
  return handleResponse<DatasourceSchemaResponse>(response)
}
